...
const jwtValidation = (req, res, next) => {
    const tmst = Date.now();
    try {
        console.log(`[${tmst}] Incoming request to be validated to: `, req?.url ?? 'not obtained');
        ...
        if (auth) {
            ...
            console.info(`[${tmst}] JWT Middleware - validated token for user: ` + decoded.user);
        }
        else throw new Error("token not found");
    }
    catch (err) {
        console.info(`[${tmst}] JWT Middleware - error validating token\n` + err);
        res.sendStatus(401);
        return res.end();
    }
    next();
...
app.listen(5000, () => console.log(`listening on http://localhost:${process.env.API_PORT}`));